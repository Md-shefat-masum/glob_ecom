...
@section('content')
    ...
    <style>
        ...
        /* Make some sections span full width (pure page-wide) */
        .page-full {
            width: 100%;
            margin-top: 18px;
        }
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="full-section">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2 col-sm-6 col-12 mb-3">
                            <div class="card card-analytics p-3">
                                <div class="kpi">
                                    <div class="left"><i class="fas fa-box-open"></i>
                                        <div>
                        ...
                        <div class="col-md-2 col-sm-6 col-12 mb-3">
                            <div class="card card-analytics p-3">
                                <div class="kpi">
                                    <div class="left"><i class="fas fa-shopping-cart"></i>
                                        <div>
                        ...
                        <div class="col-md-2 col-sm-6 col-12 mb-3">
                            <div class="card card-analytics p-3">
                                <div class="kpi">
                                    <div class="left"><i class="fas fa-undo-alt"></i>
                                        <div>
                        ...
                        <div class="col-md-2 col-sm-6 col-12 mb-3">
                            <div class="card card-analytics p-3">
                                <div class="kpi">
                                    <div class="left"><i class="fas fa-recycle"></i>
                                        <div>
                        ...
                        <div class="col-md-2 col-sm-6 col-12 mb-3">
                            <div class="card card-analytics p-3">
                                <div class="kpi">
                                    <div class="left"><i class="fas fa-truck-loading"></i>
                                        <div>
                        ...
                        <div class="col-md-2 col-sm-6 col-12 mb-3">
                            <div class="card card-analytics p-3">
                                <div class="kpi">
                                    <div class="left"><i class="fas fa-warehouse"></i>
                                        <div>
                    ...
                    <!-- Optional other analytics cards -->
                    <div class="row mt-3">
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="full-section">
                    ...
                    <div class="row">
                        <div class="col-md-3 col-sm-6 mb-3">
                            <div class="card p-3 card-analytics">
    ...
    <script>
        (function () {
            ...
            function updateKpis(payload) {
                ...
                setText('openingStock', formatNumber(inventory.opening_stock ?? 0, 2));
                setText('salesQty', formatNumber(orders.items_sold ?? 0));
                setText('returns', formatCurrency(returns.total_return_value ?? 0));
                setText('stockOut', formatNumber(inventory.stock_out ?? 0, 2));
                setText('purchaseSpend', formatCurrency(finance.purchase_spend ?? 0));
                setText('closingStock', formatNumber(inventory.closing_stock ?? 0, 2));

                setText('totalOrders', formatNumber(orders.total_orders ?? 0));
                setText('posVsEcom', `${formatNumber(orders.pos_orders ?? 0)} / ${formatNumber(orders.ecommerce_orders ?? 0)}`);
                setText('avgOrderValue', formatCurrency(orders.avg_order_value ?? 0));
                setText('returnRate', `${formatNumber(orders.return_rate ?? 0, 2)}%`);
                setText('repeatRate', `${formatNumber(orders.repeat_customer_rate ?? 0, 2)}%`);
                setText('avgItemsPerOrder', formatNumber(orders.avg_items_per_order ?? 0, 2));

                const activeUsers = Number(people.active_users ?? 0);
                setText('activeUsers', formatNumber(activeUsers));
                const conversionRate = activeUsers > 0 ? ((orders.total_orders ?? 0) / activeUsers) * 100 : 0;
                setText('conversion', `${formatNumber(conversionRate, 2)}%`);
                setText('repeatCustomerCount', `${formatNumber(people.repeat_customers ?? orders.repeat_customer_count ?? 0)} / ${formatNumber(people.distinct_customers ?? orders.distinct_customers ?? 0)}`);
            ...
            function renderTables(tables) {
                ...
                if (trendingBody) {
                    ...
                    if (!items.length) {
                        trendingBody.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No sales recorded in this range.</td></tr>';
                    } else {
                        items.forEach(item => {
                            trendingBody.innerHTML += `<tr><td>${item.rank}</td><td>${item.name}</td><td>${formatNumber(item.sold)}</td><td>${formatCurrency(item.revenue)}</td></tr>`;
                        });
                    }
                }
                ...
                if (lowStockBody) {
                    ...
                    if (!lowItems.length) {
                        lowStockBody.innerHTML = '<tr><td colspan="4" class="text-center text-muted">All stocks look healthy.</td></tr>';
                    } else {
                        lowItems.forEach(item => {
                            lowStockBody.innerHTML += `<tr class="low-stock"><td>${item.sku ?? 'N/A'}</td><td>${item.name}</td><td>${formatNumber(item.stock, 2)}</td><td>${formatNumber(item.reorder_level ?? 0)}</td></tr>`;
                        });
                    }
                }
                ...
                if (supplierBody) {
                    ...
                    if (!suppliers.length) {
                        supplierBody.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No purchasing activity in this range.</td></tr>';
                    } else {
                        suppliers.forEach(item => {
                            supplierBody.innerHTML += `<tr><td>${item.rank}</td><td>${item.supplier}</td><td>${formatNumber(item.orders)}</td><td>${formatCurrency(item.spend)}</td></tr>`;
                        });
                    }
                }
            }
            ...
            function renderPredictions(predictionData) {
                ...
                if (!enabled) {
                    ...
                    tableBody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">Enable analytics predictions to view recommendations.</td></tr>';
                ...
                if (!items.length) {
                    tableBody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">No predictions available. Run <code>php artisan analytics:predict-demand</code>.</td></tr>';
                } else {
                    tableBody.innerHTML = '';
                    items.slice(0, 10).forEach(item => {
                        const growth = item.predicted_growth_pct ?? 0;
                        const directionClass = growth > 0 ? 'text-success' : growth < 0 ? 'text-danger' : 'text-muted';
            ...
            function renderStatusBreakdown(breakdown) {
                ...
                if (posList) {
                    ...
                    if (!entries.length) {
                        ...
                        entries.forEach(([label, count]) => {
                            posList.innerHTML += `<li class="list-group-item d-flex justify-content-between align-items-center"><span>${label}</span><span class="badge badge-primary badge-pill">${formatNumber(count)}</span></li>`;
                        });
                    }
                }
                ...
                if (ecomList) {
                    ...
                    if (!entries.length) {
                        ...
                        entries.forEach(([label, count]) => {
                            ecomList.innerHTML += `<li class="list-group-item d-flex justify-content-between align-items-center"><span>${label}</span><span class="badge badge-primary badge-pill">${formatNumber(count)}</span></li>`;
                        });
                    }
                }
            }
            ...
            function renderCharts(charts) {
                ...
                const incomeData = charts.income_vs_sales || { labels: [], income: [], sales_qty: [] };
                ...
                const visitorData = charts.visitor_trend || { labels: [], visitors: [] };
                ...
                const yearData = charts.year_to_date_sales || { labels: [], totals: [] };
                ...
                const channelData = charts.channel_sales_split || { labels: [], pos: [], ecommerce: [] };
            ...
            async function fetchAnalytics() {
                ...
                setLoading(true);
                try {
                    const response = await axios.get(analyticsUrl, { params });
