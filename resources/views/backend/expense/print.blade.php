...
<head>
    <meta charset="utf-8">
    <title>Expense Voucher - #{{ $expense->expense_code }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="voucher-container">
        ...
        <div class="voucher-details">
            <div class="detail-row">
                ...
                <span class="detail-value"><strong>{{ $expense->expense_code }}</strong></span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Date:</span>
                <span class="detail-value">{{ \Carbon\Carbon::parse($expense->expense_date)->format('d M Y') }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Expense For:</span>
                <span class="detail-value">{{ $expense->expense_for }}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Category:</span>
                <span class="detail-value">{{ $expense->expense_category ? $expense->expense_category->category_name : 'N/A' }}</span>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $expense->reference_no }}</span>
            </div>
            @endif
            @if($expense->note)
            <div class="detail-row">
                <span class="detail-label">Note:</span>
                <span class="detail-value">{{ $expense->note }}</span>
            </div>
            @endif
        </div>
        ...
        <div class="amount-section">
            <div class="amount-in-numbers">
                Amount: ৳ {{ number_format($expense->expense_amt, 2) }}
            </div>
            <div class="amount-in-words">
                {{ numberToWords($expense->expense_amt) }} Taka Only
            </div>
        </div>
